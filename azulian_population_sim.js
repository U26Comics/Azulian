// Azulian Life-Cycle & Population Simulators — Cleaned Final Build
// Fully Carrd/GitHub compatible. No duplicates, no HTML tags.
// Author: U26Comics | 2025
...
// ---------- Helpers ----------
    _lifeCap(){
      const base = this.s.proven ? this.p.baseLifeIfProven
                                 : this.p.baseLifeIfNeverProven;
    ...
    _checkHusbands(){
      ...
      if(this.s.husbands>=2){
        ...
        if(Math.random()<jealousP){
          ...
          this._addLog("jealousyKill",
            "One husband killed another out of jealousy.");
        }
      }
    },
...
// ---------- Status rendering ----------
    ...
    reset(){
      this._initLife();
      this._renderStatus();
      this._newCycle(); // add new collapsible section, keep history
    }
  ...
  function boot(){
    ...
    if(document.getElementById(id))
      window.AzulianLifeSim.mount(id);
  }
